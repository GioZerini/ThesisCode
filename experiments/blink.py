...
from acds.benchmarks import get_blink_data
...
parser.add_argument("--gamma_range", type=float)
parser.add_argument("--epsilon_range", type=float)
...
if args.trials > 1:
    ...
    train_loader, valid_loader, test_loader = get_blink_data(args.batch, args.batch, whole_train = True)
else:
    train_loader, valid_loader, test_loader = get_blink_data(args.batch, args.batch)
...
if args.ron:
    f = open(os.path.join(args.resultroot, f"Blink_log_RON_{args.topology}{args.resultsuffix}.txt"), "a")
else:
    raise ValueError("Wrong model choice.")
